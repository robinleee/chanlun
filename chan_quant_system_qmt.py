...
class ChanEngine:
    ...
    def push_raw_kline(self, ts, open_p, high_p, low_p, close_p):
        ...
        if inc_result is None:
            return "NO_SIGNAL"
        sk = StandardKLine(
            index=len(self.sklines), open=round_price(inc_result['open']),
            high=round_price(inc_result['high']), low=round_price(inc_result['low']),
            close=round_price(inc_result['close']), direction=inc_result['direction'])
    ...
    def _update_geometry_topology(self):
        ...
        if N >= 4:
            for i in range(max(1, self._last_scanned_fractal_idx), N - 3):
                prev, curr, nxt = sklines[i-1], sklines[i], sklines[i+1]
                f = None
                    ...
                    if not self._last_bi_end_fractal:
                        ...
                        if StrictBiValidator.validate(self._last_bi_end_fractal, f):
                            ...
                            print(f"[GEOMETRY] Confirmed New Bi: {self.stock_code} [{self.period}] | "
                                  f"Direction={new_bi.direction} | Start Fractal={new_bi.start.index} "
                                  f"({new_bi.start.type}) at {new_bi.start.high if new_bi.start.type == 'TOP' else new_bi.start.low:.2f} | "
        ...
        if self.current_zhongshu and self.active_segment and self.active_segment.direction == "UP":
            ...
            if latest_close >= self.current_zhongshu.GG * 1.20:
                for f in self._confirmed_fractals:
        ...
        if self.current_zhongshu and bis:
            latest_bi = bis[-1]
            latest_close = sklines[-1].close
            zg, zd = self.current_zhongshu.ZG, self.current_zhongshu.ZD
            if zg > zd:
    ...
    def submit_order(self, stock_code, price, volume, side, order_id, strategy_name="CHAN_FSM"):
        if self.is_live:
            try:
                ...
                self.client.order_stock(symbol=stock_code, price=price, volume=volume,
            ...
            self.controller.handle_trade_callback(stock_code=stock_code, side=side,
                volume=volume, price=price, order_id=order_id, traded_time=traded_time)
    ...
    def on_bar(self, stock_code, timeframe, timestamp, open_p, high_p, low_p, close_p,
        ...
        with lock:
            ...
            if not disable_trading:
                ...
                fsm.update_state(signal, close_p, can_sell,
                    small_to_large_meltdown=is_meltdown, order_id=order_id, bar_ts=timestamp)
            ...
            if timeframe == self.micro_period:
                ...
                for c in self.full_pool:
                    ...
                    c_close = self.engines_5m[c].sklines[-1].close if self.engines_5m[c].sklines else close_p
...
class BacktestDriver:
    ...
    def run(self):
        ...
        try:
            print(f"[BACKTEST_CONFIG] start_date={self.start_date} | "
                  f"end_date={self.end_date} | macro_period={self.macro_period} | "
            ...
            cfg_items = [f"{k}={v}" for k, v in cfg.items()]
    ...
    def _fallback_report(self):
        ...
        for code, fsm in kernel.fsm_accounts.items():
            if fsm.held_shares > 0:
                close = kernel.engines_5m[code].sklines[-1].close if kernel.engines_5m[code].sklines else 0.0
    ...
    def _fetch_data_with_fallback(self, code, period, threshold, preheat_start, end_date):
        raw = self.client.get_local_data(
            fields=['open', 'high', 'low', 'close'], symbols=[code],
            period=period, start_time=preheat_start, end_time=end_date)
        ...
        if self._check_data_sufficient(raw, code, threshold):
            return raw
        print(f"[{code}] {period} 本地缓存仍不足 ({self._count_data(raw, code)} 条)，切换为在线拉取...")
    ...
    def _build_timeline(self):
        ...
        all_bars_5m, all_bars_1m = {}, {}
        for code in self.stock_pool:
            raw_5m = self._fetch_data_with_fallback(code, self.macro_period, threshold_macro,
                self.preheat_start_date, self.end_date)
            raw_1m = self._fetch_data_with_fallback(code, self.micro_period, threshold_micro,
                self.preheat_start_date, self.end_date)
        ...
        for code in self.stock_pool:
            for bars_dict in [all_bars_5m, all_bars_1m]:
                if code in bars_dict and hasattr(bars_dict[code], 'index'):
                    all_timestamps.update(bars_dict[code].index)
        all_timestamps = sorted(all_timestamps)
        ts_5m = sum(1 for c in self.stock_pool if c in all_bars_5m and hasattr(all_bars_5m[c], 'index'))
        ts_1m = sum(1 for c in self.stock_pool if c in all_bars_1m and hasattr(all_bars_1m[c], 'index'))
    ...
    def _playback(self, all_bars_5m, all_bars_1m, all_timestamps):
        ...
        for ts in all_timestamps:
            ...
            for code in self.kernel.active_pool:
                is_5m = code in all_bars_5m and ts in all_bars_5m[code].index
                is_1m = code in all_bars_1m and ts in all_bars_1m[code].index
                if is_5m:
                    row = all_bars_5m[code].loc[ts]
                    self.kernel.on_bar(code, self.macro_period, ts,
                        float(row['open']), float(row['high']),
                        float(row['low']), float(row['close']), disable_trading=is_preheating)
                if is_1m:
...
class LiveDriver:
    ...
    def _handle_sse_event(self, event):
        ...
        if etype == 'bar':
            ...
            self.kernel.on_bar(code, period, ts,
...
class SystemController:
    def __init__(self, stock_pool, execution_mode: str, client: QMTBridgeClient,
        ...
        self.gateway = QMTBridgeTraderGateway(client=client, controller=None, is_live=is_live_flag)
    ...
    def _daily_trend_filter(self, pool, current_date=None):
        ...
        for code in pool:
            ...
            if ma5 > ma20 and closes.iloc[-1] >= 0.92 * ma20:
                if (closes.iloc[-1] / closes.iloc[-20] - 1) > -0.08:
    ...
    def _fundamental_filter(self, pool):
        ...
        for code in pool:
            ...
            if isinstance(fin, dict) and 'debt_ratio' in fin:
                try:
                    if float(fin.get('debt_ratio', 100)) > 85:
                        continue
                except (ValueError, TypeError):
                    pass
            if isinstance(fin, dict):
                try:
                    if float(fin.get('net_profit', 0)) < 0:
                        continue
                except (ValueError, TypeError):
                    pass
    ...
    def recompute_active_pool(self, bar_date_str):
        try:
            ...
            if not current_pool:
                return []
            l1 = self._fundamental_filter(current_pool) or current_pool
    ...
    def _fallback_rs_filter(self, pool, current_date=None):
        rs_values = {}
        if pool and hasattr(self, '_all_daily_closes_cache') and self._all_daily_closes_cache is not None:
            for code in pool:
